...
本平台在学术层面参考了六篇研究论文（见 `原版文件/` 目录），实现了 Graph-SDP、GCC-SDP、GS-LDP、NDKD、VPCS、zkGCN 的核心思想与主要流程，并通过交互式 Web 界面呈现数据治理、可信查询、可验证推理和审计追踪的关键步骤。平台采用前后端分离架构，后端基于 FastAPI + SQLAlchemy + SQLite，前端基于 React 18 + TypeScript + Vite + Tailwind CSS，是一套兼具算法深度、治理闭环与工程完整性的企业级平台原型。

> **技术边界说明**：
> - **Graph-SDP / GCC-SDP / GS-LDP / NDKD**：当前工程版本覆盖论文核心流程（ESA 框架、k-RR、SUC 编码、度分布/三角计数/聚类系数采集、k-度匿名等），部分推导细节与原论文相比做了工程化简化。
> - **VPCS 加密路径查询**：当前版本完整覆盖四角色协作、约束路径搜索、虚假边混淆、证明摘要与完整性校验；底层高安全等级密码原语以 SHA-256 哈希承诺方式进行工程化验证，不等同于生产级 Threshold Paillier 或双线性映射实现。
> - **zkGCN 可验证推理**：当前版本完整覆盖真实 GCN 前向推理、分层见证摘要、proof/vk/pk 哈希链与完整性校验；零知识证明部分以哈希承诺方式进行工程化验证，不等同于生产级 zk-SNARK/Groth16 与真实 R1CS 电路实现。
...
| analyst | analyst123 | 数据分析师 | 可发起隐私计算、可信查询和风险分析任务 |
| auditor | auditor123 | 审计专员 | 可执行审计校验、风险复核和合规核验 |
| observer | observer123 | 业务观察员 | 只读审阅权限，用于方案评估、业务巡检和结果复核 |
...
### 1. 总览驾驶舱（Dashboard）
平台数据资产总量、风险告警数、隐私计算任务数、合约活跃数等核心指标实时展示，提供平台全局状态的一目了然视图，是治理运营与联调核验的总入口。
...
集成四大图差分隐私算法（Graph-SDP、GCC-SDP、GS-LDP、NDKD），支持在线参数调节（隐私预算 ε、k 值等），可视化展示原始分布、加噪分布、校正分布三条曲线，并逐步解析每个算法环节，兼顾算法严谨性与业务可解释性。

### 4. 加密路径查询 VPCS（VPCS Query）
覆盖 GO/CS/Proxy/QU 四角色协议交互，对受保护图进行约束最短路径查询（含多维约束：距离、费用、时间、预算），展示哑边混淆、证明摘要与完整性校验全流程，并提供异常校验场景以验证可验证链路的防护效果。

### 5. 可验证推理 zkGCN（ZK-GCN）
对图卷积网络（GCN）的推理过程构建可验证证明链路，展示前向推理、分层见证摘要、proof/vk/pk 哈希生成与验证步骤，并提供异常校验场景以展示证明失效时的拦截效果。
...
### 9. 审计追踪（Audit Trail）
全平台操作行为写入哈希链式不可篡改审计日志，每条日志记录前驱哈希值形成链式结构，提供链完整性一键校验功能，并支持异常日志校验场景，用于验证任意节点被修改后的告警与定位能力。
...
├── DEMO_SCRIPT.md             # 平台讲解稿
├── COMPETITION_HIGHLIGHTS.md  # 平台亮点与创新点
│
├── 原版文件/                   # 参考学术论文（PDF）
...
隐私计算实验室 → Graph-SDP（ε=1.0）→ 查看三条分布曲线
                → NDKD（k=3）→ 查看匿名图输出
    ↓
VPCS 可信路径查询 → 四角色交互 → 异常校验场景
    ↓
zkGCN 可验证推理 → 证明生成 → 异常校验场景
    ↓
行业解决方案 → 金融联合风控流程执行
    ↓
审计追踪 → 哈希链验证 → 异常日志校验
...
1. **密码学模块为工程化验证实现**：VPCS 的加密操作、zkGCN 的零知识证明当前以 HMAC/SHA256 哈希承诺方式完成工程化验证，用于支撑协议流程和完整性校验；生产环境需替换为正式密码学库与证明系统。

2. **数据库初始化**：首次启动后端时，`init_db()` 会自动创建所有数据表并写入种子数据，无需手动迁移。

3. **数据重置**：如需恢复干净的本地基线数据，可执行 `bash scripts/reset_demo_data.sh`。脚本会先备份当前 SQLite 数据库，再重建库表并重新注入种子数据。

4. **CORS 配置**：后端当前允许全部来源（`allow_origins=["*"]`），仅适用于本地开发与联调环境，生产部署须限制来源。

5. **隐私预算建议**：联调或汇报时 ε 推荐取 0.5～2.0，过小（如 0.1）会导致噪声极大、曲线失真，不利于观察效用保持效果。

6. **SQLite 并发限制**：SQLite 不支持高并发写入，当前版本定位为单机验证与联调环境，如需生产级部署建议替换为 PostgreSQL。
